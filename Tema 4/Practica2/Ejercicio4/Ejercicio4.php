...
<body>
    <header>
        <h1 style='text-shadow: 2px 2px #ff0000; color:black '>Ejercicio 4</h1>
        <a href="#addProducto" class="btn btn-warning" data-toggle="modal" style="position:absolute; top: 4%; right: 11%"><span>Añadir Producto</span></a>
    ...
    function leerArchivo($fichero)
    {
        ...
        foreach ($datos as $valor) {
            ...
            $tienda[] = array("ISBN" => $producto[0], "titulo" => $producto[1], "descripcion" => $producto[2], "categoria" => $producto[3], "editorial" => $producto[4], "foto" => $producto[5], "precio" => $producto[6]);
        }
    ...
    function escribirArchivo($tienda)
    {
        $file = fopen("tienda.txt", "w");
        if (flock($file, LOCK_EX)) {
            foreach ($tienda as $producto) {
                fwrite($file, $producto['ISBN'] . "@" . $producto['titulo'] . "@" . $producto['descripcion'] . "@" . $producto['categoria'] . "@" . $producto['editorial'] . "@" . $producto['foto'] . "@" . $producto['precio'] . "|");
            }
        }
        fflush($file);
        flock($file, LOCK_UN);
    ...
    function pintarTienda($tienda)
    {
        ...
        foreach ($tienda as $libro) {
            ...
            echo "<a href='Ejercicio4.php?borrar=" . $libro['ISBN'] . "' style='position:absolute; top:1%; right:2%;'><span aria-hidden='true'>&times;</span></a>";
            echo '<div class="card-body">';
            echo '<img src="img/' . $libro["foto"] . '" class="card-img-top" alt="...">';
            echo '<hr/>';
    ...
    $_SESSION['tienda'] = leerArchivo('tienda.txt');

    if (isset($_POST)) {
        if (isset($_POST['nombre'])) {
            foreach ($_SESSION['tienda'] as $libro) {
        ...
        if (isset($_POST['add'])) {
            if (!in_array($_POST['ISBN'], array_column($_SESSION['tienda'], 'ISBN'))) {
                file_put_contents("tienda.txt", strtolower($_POST['ISBN']) . "@" . strtolower($_POST['titulo']) . "@" . strtolower($_POST['descripcion']) . "@" . strtolower($_POST['categoria']) . "@" . strtolower($_POST['editorial']) . "@" . strtolower($_POST['foto']) . "@" . strtolower($_POST['precio']) . "|", FILE_APPEND | LOCK_EX);
            }
        }
    }

    if (isset($_GET['borrar'])) {
        if (in_array($_GET['borrar'], array_column($_SESSION['tienda'], 'ISBN'))) {
            $posicion = array_search($_GET['borrar'], array_column($_SESSION['tienda'], 'ISBN'));
            unset($_SESSION['tienda'][$posicion]);
            escribirArchivo($_SESSION['tienda']);
        }
    }
            ...
            <div class="modal-content">
                <form method='POST'>
                    <div class="modal-header">
                        <h4 class="modal-title" id="exampleModalLabel">Introducir Producto</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <h5 class="modal-title">Introduzca todos los datos de este formulario</h5><br />
                        ...
                        <label for="disabledTextInput">Foto</label>
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="submit" name='add' class="btn btn-primary" value="Añadir">
                    </div>
                </form>
            </div>
        </div>
